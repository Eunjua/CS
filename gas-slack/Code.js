 ...
 *
 * 알림 안 보내는 경우
 *   G(처리 날짜)가 이미 찍힌 줄 : 끝난 요청이라 E·B·C·D를 고쳐도 알림 없음
 *
...
function 시트수정감지(e) {
  ...
  const 열 = 셀.getColumn();
  if (열 !== 처리사항열 && 요청칸들.indexOf(열) === -1) return;

  // 처리 날짜가 이미 있으면 끝난 요청. 내용을 고쳐도 알림은 다시 보내지 않는다
  if (String(셀.getSheet().getRange(셀.getRow(), 처리날짜열).getValue()).trim()) return;
