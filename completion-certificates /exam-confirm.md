---
name: exam-confirm
description: 시험응시 확인서를 생성하고 PDF로 저장. 사용자가 응시 확인서를 요청하거나 /exam-confirm을 호출하면 실행.
argument-hint: "[응시자이름]"
---

## 역할

사용자와 대화를 통해 정보를 수집한 뒤, 시험응시 확인서를 PDF로 생성하여 저장한다.
...
- `$ARGUMENTS`가 있으면 그것을 응시자 이름으로 사용
- 없으면 먼저 질문
...
2. **시험명** — 응시한 시험 이름
3. **응시일** — 시험 날짜 (예: 3월 13일)
4. **응시방법** — 기본값은 `온라인` (다른 경우만 변경)
...
# ── 입력값 ──────────────────────────────────────────────
applicant_name = "{{응시자이름}}"
birth_date_raw = "{{생년월일}}"  # YYYYMMDD
exam_name = "{{시험명}}"
exam_date = "{{응시일}}"         # 예: 2026년 03월 13일
exam_method = "{{응시방법}}"     # 예: 온라인
...
output_path = os.path.join(output_dir, f"확인서_{applicant_name}.pdf")
issue_date = date.today().strftime("%Y년 %m월 %d일")

# ── 생년월일 포맷 변환 ───────────────────────────────────
y, m, d = birth_date_raw[:4], birth_date_raw[4:6], birth_date_raw[6:8]
birth_formatted = f"{y}년 {m}월 {d}일"
...
# 응시자 정보
c.setFont("KR", 12)
c.drawString(margin, h - 7.0 * cm, "■  응시자 정보")
c.setFont("KR", 11)
...
# 시험 정보
c.setFont("KR", 12)
c.drawString(margin, h - 11.7 * cm, "■  시험 정보")
c.setFont("KR", 11)
c.drawString(margin + 0.5*cm, h - 13.0 * cm, f"시  험  명  :  {exam_name}")
c.drawString(margin + 0.5*cm, h - 14.3 * cm, f"응시일시  :  {exam_date}")
c.drawString(margin + 0.5*cm, h - 15.6 * cm, f"응시방법  :  {exam_method}")

c.setLineWidth(1.2)
c.line(margin, h - 16.6 * cm, w - margin, h - 16.6 * cm)

# 확인 문구
confirm = "위 사람은 상기 시험에 응시하였음을 확인합니다."
c.setFont("KR", 11)
